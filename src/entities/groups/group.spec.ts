    ...
    },
    {
        ...
        "name": "QA"
            ...
            it('should return group with ID', async()=> {
                ...
                const next = jest.fn();
                group.Group.findOne.mockImplementation((arg: {where: {id:string}})=> Promise.resolve(groups.find(group => group.id === arg.where.id)));
            ...
            it('should return not-found if there is no group with ID', async()=> {
                ...
                group.Group.findOne.mockImplementation((arg: {where: {id:string}})=> Promise.resolve(groups.find(group => group.id === arg.where.id)));
                await GroupService.getGroupById(req as any, res as any, next);
                expect(next).toBeCalledWith({...res.error, module: 'GroupService', method: 'getGroupById'});
            }) 
